...
export class AgentPerformanceReportComponent implements OnInit {
  ...
  constructor(private ReportService: ReportService, private excelservice: ExcelService) {
    this.AgentPerformanceData = new Array<any>();
  }
  ...
  ngOnInit(): void {
    ...
    this.subscription = this.ReportService.observeReportData().subscribe(message => {
      ...
      this.ReportService.postData('AgentPerformanceReport',Reqparameter).subscribe(data => {
      // this.ReportService.getAgentPerformanceReport(Reqparameter).subscribe(data => {
        if (data.data) {
          this.AgentPerformanceData = data.data;
          this.totalRec = data.data.length;
          console.log('AgentPerformanceData', this.AgentPerformanceData);
        }
        else {
  ...
  exportexcel(): void {
    this.excelservice.exportAsExcelFile(this.AgentPerformanceData, 'AgentPerformanceReport');
  }
